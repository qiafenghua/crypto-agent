...
//            }
//            logger.info("[CoingeckoTool] 获取代币详细信息成功，请求结果: " + response.body().string());
...
//        try (Response response = client.newCall(request).execute()) {
//            if (!response.isSuccessful()) {
//                logger.error("[CoingeckoTool] 搜索失败: " + response);
//                return "搜索失败加密货币失败，失败原因： " + response.message();
//            }
//            logger.info("[CoingeckoTool] 搜索成功，请求结果: " + response.body().string());
//            return mapper.readTree(response.body().string()).toPrettyString();
//        }
//    }
//
//    @Tool(name = "coingecko_get_market_chart",
//            description = "获取指定币种的历史价格数据（天级）")
//    public String getMarketChart(@ToolParam(name = "coin_id") String coinId,
//                                 @ToolParam(name = "days", description = "天数：7, 30, 90, 365") String days) throws IOException {
...
//            }
//            logger.info("[CoingeckoTool] 请求CoinGecko API成功，请求结果: " + response.body().string());
//            return mapper.readTree(response.body().string()).toPrettyString();
//        }
//    }
//}